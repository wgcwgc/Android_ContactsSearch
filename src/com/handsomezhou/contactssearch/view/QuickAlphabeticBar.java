...
public class QuickAlphabeticBar extends View
{
	...
	private static char [] mSelectCharacters =
	{ DEFAULT_INDEX_CHARACTER, 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I',
	...
	public void setOnQuickAlphabeticBar(
	        OnQuickAlphabeticBar onQuickAlphabeticBar )
	{
	...
	public boolean onTouchEvent(MotionEvent event )
	{
		        ...
		        || (event.getAction() == MotionEvent.ACTION_MOVE))
		{
			...
			{
				int position = mSectionIndexer
				        .getPositionForSection(mSelectCharacters[index]);
			...
			{
				mOnQuickAlphabeticBar
				        .onQuickAlphabeticBarDown(mSelectCharacters[index]);
			}
			...
			if(event.getAction() == MotionEvent.ACTION_UP)
			{
				...
				{
					mOnQuickAlphabeticBar
					        .onQuickAlphabeticBarUp(mSelectCharacters[index]);
				}
			}
		return true;
	}
	...
	protected void onDraw(Canvas canvas )
	{
		...
		if(mSelectCharacters.length > 0)
		{
			...
			{
				// xPos=getMeasuredWidth()/2-
				// mOtherIndexPaint.measureText(String.valueOf(mSelectCharacters[i]));
				yPos = (i + 1) * sigleHeight;
				canvas.drawText(
				        String.valueOf(mSelectCharacters[i]) ,
				        xPos ,
				        yPos ,
				        mSelectCharacters[i] == mCurrentSelectChar ? mCurrentIndexPaint
				                : mOtherIndexPaint);
			}
		}
	...
	private int getCurrentIndex(MotionEvent event )
	{
		...
		if(index < 0)
		{
			...
			if(index >= mSelectCharacters.length)
			{
